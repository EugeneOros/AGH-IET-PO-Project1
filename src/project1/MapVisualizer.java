...
public class MapVisualizer {
    ...
    private static final String FRAME_SEGMENT = "__";
    ...
    private String drawObject(Vector2d currentPosition) {
        ...
        if (this.map.isOccupied(currentPosition)) {
            ...
            elements = this.map.objectsAt(currentPosition);
            if (elements != null) {
                if (elements.size() == 1) {
                    result = EMPTY_CELL + elements.get(0).toString() + EMPTY_CELL;
                } else {
                    result = elements.size() + EMPTY_CELL + elements.get(1).toString();
                }
            } else {
                result = "\t";
            }
